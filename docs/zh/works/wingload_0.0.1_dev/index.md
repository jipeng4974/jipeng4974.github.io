...
[Wingload 0.0.1](/zh/works/wingload_demo_0.0.1/) 的开发全程由 AI agent 驱动：代码是 agent 写的，但 agent 没法亲手玩游戏。这条约束决定了整套工作流的形态——游戏必须自带"可被脚本驾驶"的测试钩子，否则 agent 只能说"代码写完了"，不能说"游戏能玩"。这篇文章把 0.0.1 开发中的三条主线整理成文：**无头测试 harness**、**C# → GDScript 全量移植**、**导出与运行时验证**，最后附一份实战踩坑清单。
