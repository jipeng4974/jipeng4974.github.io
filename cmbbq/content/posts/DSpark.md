...
Confidence head就是单个线性层，每位置输出标量$c_k$，建模条件存活概率（前$k-1$个全被接受的条件下第$k$个被接受的概率）。监督信号是解析的：逐步接受率恰好等于$1 - \tfrac{1}{2}\|p_d - p_t\|_1$，直接BCE训练，无需额外标注。神经置信度系统性过自信，post-hoc用逐位置温度缩放（STS）校准累积乘积的ECE，从3%~8%降到~1%；温度缩放保序，不破坏token排序。
...
在线（DeepSeek-V4-Flash/Pro preview，真实流量，对比生产基线MTP-1）：80 tok/s/user SLA下聚合吞吐+51%；吞吐匹配时单用户速度+60%~85%。负载自适应符合预期：中低并发时验证预算从静态2 token扩到4~6 token，并发饱和时平滑收缩。论文里+661%这类数字，我建议take it with a grain of salt[^6]。
